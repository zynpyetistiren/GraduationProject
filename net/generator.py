...
def Unsample(inputs, filters, kernel_size=3):
    '''
        ...
        '''
    new_H, new_W = 2 * inputs.shape[1].value, 2 * inputs.shape[2].value
...
def Downsample(inputs, filters = 128, kernel_size=3):
    '''
        An alternative to transposed convolution where we first resize, then convolve.
        See http://distill.pub/2016/deconv-checkerboard/
        For some reason the shape needs to be statically known for gradient propagation
    ...
    def __init__(self, inputs):
        ...
        with tf.compat.v1.variable_scope('G_MODEL'):
            with tf.variable_scope('b1'):
                ...
                inputs = Conv2DNormLReLU(inputs, 32)
                inputs = Separable_conv2d(inputs,64,strides=2) + Downsample(inputs, 64)

            with tf.variable_scope('b2'):
                inputs = Conv2DNormLReLU(inputs, 64)
                inputs = Separable_conv2d(inputs, 64)
            ...
            with tf.variable_scope('m'):
                ...
                inputs = self.InvertedRes_block(inputs, 2, 64, 1, 'r1')
                inputs = self.InvertedRes_block(inputs, 2, 64, 1, 'r2')
                inputs = self.InvertedRes_block(inputs, 2, 64, 1, 'r3')
                inputs = self.InvertedRes_block(inputs, 2, 64, 1, 'r4')
                inputs = self.InvertedRes_block(inputs, 2, 64, 1, 'r5')
                inputs = self.InvertedRes_block(inputs, 2, 64, 1, 'r6')
                inputs = self.InvertedRes_block(inputs, 2, 64, 1, 'r7')
                inputs = self.InvertedRes_block(inputs, 2, 64, 1, 'r8')
                inputs = Conv2DNormLReLU(inputs, 64)
